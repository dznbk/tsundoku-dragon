---
description: 新機能を既存パターンに従って、実装する
---
...
2. `docs/`, `planning/` ディレクトリ内の永続ドキュメントを確認し、関連する設計思想やアーキテクチャを理解する。

## ステップ3: 既存パターンの調査

1. Grepツールを使用し、機能名に関連するキーワードでソースコードを検索する。以下はバックエンド実装時の例

```bash
Grep('[機能に関連するキーワード]', 'apps/api/')
```
...
## ステップ9: Pull Request作成

1. 実装した変更をコミットする（未コミットの変更がある場合）。

2. `gh pr create`コマンドを使用してPull Requestを作成する。

- タイトル: 機能名を含めた簡潔な説明
- 本文: 実装内容のサマリ、テスト方法を含める

3. 作成したPRのURLをユーザーに報告する。
