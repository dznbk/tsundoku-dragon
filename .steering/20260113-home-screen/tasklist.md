...
- 「本を登録する」ボタンがファーストビューに入らない
- 常時画面内に表示されていた方が利便性が高い
- 画面下部固定バーで実装（将来の拡張性を考慮）

### 追加タスク

- [x] BottomActionBar コンポーネントを新規作成
- [x] HomePage.tsx で BottomActionBar を使用
- [x] 既存の actions セクションを削除
- [x] 品質チェック（lint, typecheck, format:check）

### 振り返り

- 完了日: 2026-01-14
- 対応内容: 「本を登録する」ボタンを画面下部固定バーに移動
- 学んだこと: 将来の拡張性を考慮して汎用コンポーネント（BottomActionBar）として実装
