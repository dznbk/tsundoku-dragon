...
## 追加対応 (2026-01-11) - CORS修正

### フィードバック内容

- 本の登録でPOSTリクエストが400エラー（OPTIONSプリフライトリクエストが失敗）

### 追加タスク

- [x] APIサーバーにCORSミドルウェアを追加（`apps/api/src/index.ts`）
- [x] 品質チェック（test, lint, typecheck, format:check）

### 振り返り

- 完了日: 2026-01-11
- 原因: ブラウザがカスタムヘッダー（Authorization）付きのリクエストを送る前にOPTIONSプリフライトを送信するが、APIサーバーでCORS設定がなかったため失敗
- 対応: Honoの組み込み`cors`ミドルウェアを使用してOPTIONSリクエストを適切に処理
