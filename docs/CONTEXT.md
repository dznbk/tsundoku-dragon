...
### 2026-02-11 Terraform 方針策定

**議論した内容：**

- Terraform の管理対象スコープ（プロビジョニング vs デプロイの分離）
- Firebase Auth を Terraform で管理するか
- 環境分離の方法（workspace / ディレクトリ分離 / tfvars）
- State 管理（ローカル vs リモートバックエンド）

**決定事項と理由：**

| 決定                                                                       | 理由                                                                                               |
| -------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| プロビジョニング（Terraform）とデプロイ（wrangler + GitHub Actions）を分離 | 既存の CI/CD を壊さず、Terraform の宣言的管理の恩恵を受けられる                                    |
| Firebase Auth は管理対象外                                                 | 変更頻度が低く、Terraform 管理のコスト対効果が合わない                                             |
| 環境ごとにディレクトリ分離                                                 | State が物理的に分離され誤操作リスクが低い。環境間の構成差異（Cloudflare Access 等）に対応しやすい |
| ローカル State で運用開始                                                  | 個人プロジェクトで同時操作リスクなし。必要時にリモートバックエンドへ移行可能                       |
